...
class TrainingMonitor:
    ...
    def log_epoch(self, epoch, stage_name, mode_name, avg, lr):
        ...
        msg = (f"Ep {epoch:03d} [{stage_name}|{mode_name}] | Total: {avg['Total']:.4f}\n"
    ...
    def check_and_save_best(self, current_loss, model, criterion, epoch, save_dir, stage_name):
        if current_loss < self.best_loss:
            ...
            torch.save(save_dict, os.path.join(save_dir, 'best_model_all_stages.pth'))
            self.logger.info(f"🌟 New Global Best! Ep {epoch} ({stage_name}) Loss: {current_loss:.4f}")
...
class Trainer:
    ...
    def train_epoch(self, epoch, stage_name, mode_name):
        ...
        pbar = tqdm(enumerate(self.dataloader), total=len(self.dataloader), desc=f"Ep {epoch} [{mode_name}]")
        
        acc_metrics = {k: 0.0 for k in self.monitor.headers[3:-1]} # Initialize all stats
        ...
        for i, batch in pbar:
            ...
            mu_pos, log_var_pos, mu_rot, log_var_rot, logits_pos, logits_rot = self.model(
                shape, state, timestep, tooth_types, mask,
            ...
            loss, d = self.criterion(
                mu_pos, log_var_pos, mu_rot, log_var_rot,
                logits_pos, logits_rot,
                gt_pos_mu, gt_rot_mu, 
                gt_mask_pos, gt_mask_rot,
                mask, feat_prev_pos, feat_prev_rot
            )
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(list(self.model.parameters()) + list(self.criterion.parameters()), 1.0)
            self.optimizer.step()
            self.scheduler.step()
            ...
            if i % 10 == 0: 
                pbar.set_postfix({'L':f"{d.get('Total', 0):.2f}", 'SigP':f"{d.get('Avg_Sigma_Pos', 0):.3f}"})
            
        avg = {k: v/len(self.dataloader) for k,v in acc_metrics.items()}
        return avg

    def train(self, start_epoch):
        self.monitor.logger.info(f"🚀 Starting Training Engine")
        ...
        for epoch in range(start_epoch, self.config.NUM_EPOCHS):
            ...
            self.monitor.check_and_save_best(avg_stats['Total'], self.model, self.criterion, epoch + 1, self.config.SAVE_DIR, stage_name)
