...
class SequentialFusionLoss(nn.Module):
    def __init__(self):
        super().__init__()
        # log_vars: 用于自动加权多任务 Loss (异方差不确定性加权)
        ...
        # 各项 Loss 的基础权重
        self.lambda_mse_pos = 1.0 
        self.lambda_mse_rot = 0.005 
        self.lambda_nll_default = 0.05
        self.lambda_nll_freeze = 0.01 
        self.lambda_cos = 0.05 
    ...
    def forward(self, mu_pos, log_var_pos, mu_rot, log_var_rot, logits_pos, logits_rot,
        ...
        # ==========================================
        # 🔥 核心重构：物理层保护伞 (Late-Fusion Masking)
        # ==========================================
        ...
        # 🌟 极致严谨的控制变量：所有变体统一乘以门控概率。
        # 让“静止”的牙齿物理预测归零，彻底免除梯度毒害。
        ...
        if use_nll_pos:
            ...
            term_pos = self.lambda_nll_default * l_nll_pos
        ...
        if use_nll_rot:
            ...
            lambda_nll_rot = self.lambda_nll_freeze if self.rot_freeze_mode else self.lambda_nll_default
            term_rot = lambda_nll_rot * l_nll_rot
