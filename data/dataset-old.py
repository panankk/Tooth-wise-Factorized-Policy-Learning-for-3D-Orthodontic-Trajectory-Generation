...
class OrthoDataset(Dataset):
    def __init__(self, processed_root):
        """
        V39 最终版 Dataset
        支持串行热启动训练的所有阶段 (V1/V2/V3/Fusion)
        """
        ...
        print(f"🧹 V39 Strategic Dataset (Sequential Fusion Ready): {len(self.cases)} Cases Loaded.")
    ...
    def make_combined_state(curr_p, goal_p, teeth_mask):
        """
        构建基础的共享特征 State (供 Encoder 提取全局几何特征 -> Regression Head)
        Mask Head 不再使用这个特征
        """
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 如果读取失败，跳过当前样本，随机返回另一个
            # print(f"⚠️ Error loading {case_id}: {e}")
        ...
        # 🚨 Inertia Dropout: 30% 概率触发，强制丢弃惯性信息
        # 目的：逼迫模型学习 "No Inertia + High Completion -> Wake Up"
        if torch.rand(1).item() < 0.3: 
            drop_mask = (torch.rand(32) < 0.5) 
            # 只在真实动的时候做 dropout 才有意义
        ...
        state = self.make_combined_state(curr_p, goal_p, teeth_mask) # 仅供 Regression Head 使用
        ...
        # 🎯 V39 统一输出格式
        # 无论训练阶段如何，都返回 Mu 标签。
        # Loss 函数会根据当前阶段决定是使用 MSE 还是 NLL，以及是否更新 log_var。
        
        return {
            'shape': shape_emb.view(32, 1024, 3),
            'input_seq': state.view(1, -1),
            
            # 🔥 战略通道特征 (纯逻辑，无形状，无 ID)
            'strat_vec_pos': strat_vec_pos.view(1, -1), # [1, 320] Flattened
            'strat_vec_rot': strat_vec_rot.view(1, -1), # [1, 320] Flattened
            
            # --- 辅助特征 (用于 Loss 加权) ---
            'feat_prev_pos': feat_prev_active_pos.view(1, -1),
            'feat_prev_rot': feat_prev_active_rot.view(1, -1),
            
            # --- 监督信号 (Regression Targets - Means) ---
            # 统一命名为 _mu，表示这是高斯分布的均值目标
            'gt_pos_mu': target_pos_delta.view(1, -1),       # [1, 32*3]
            'gt_rot_mu': (target_rot_delta_raw * 100.0).view(1, -1), # [1, 32*6]
            
            # --- 监督信号 (Classification Targets) ---
